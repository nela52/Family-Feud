...
public class KeyInput extends KeyAdapter
{
	...
	public void keyPressed(KeyEvent e)
	{
		...
		if (FamilyFued.stage == Stage.ENTERING)
		{
			...
			if (FamilyFued.suddenDeathRound != null)
			{
				...
				if (e.getKeyCode() == 10)
				{
					...
					canType = false;
					FamilyFued.suddenDeathRound.submitAnswer(FamilyFued.suddenDeathRound.answer);
					return;
				}
		...
		if (FamilyFued.stage == Stage.FAST_MONEY)
		{
			//Enter
			if (e.getKeyCode() == 10)
			{
				if (FamilyFued.fastMoneyRound.answer.length() < 3)
				{
					return;
				}

				FamilyFued.fastMoneyRound.submitAnswer(FamilyFued.fastMoneyRound.answer);
				FamilyFued.playSound("Key.wav");
				return;
			}
			...
			if (e.getKeyCode() == 32)
			{
				FamilyFued.fastMoneyRound.answer = FamilyFued.fastMoneyRound.answer + " ";
				FamilyFued.playSound("Key.wav");
				return;
			}
			...
			if (e.getKeyCode() == 8)
			{
				if (FamilyFued.fastMoneyRound.answer.length() == 0)
				{
					return;
				}

				FamilyFued.fastMoneyRound.answer = removeLastChar(FamilyFued.fastMoneyRound.answer);
			...
			{
				char c = Character.toUpperCase(e.getKeyChar());
				FamilyFued.fastMoneyRound.answer = FamilyFued.fastMoneyRound.answer + c;
				return;
			}

			FamilyFued.fastMoneyRound.answer = FamilyFued.fastMoneyRound.answer + e.getKeyChar();
		}
	}
